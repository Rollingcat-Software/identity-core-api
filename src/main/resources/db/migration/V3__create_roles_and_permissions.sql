...
(
    ...
    CONSTRAINT unique_resource_action UNIQUE
(
    resource,
    action
)
...
(
    ...
    CONSTRAINT unique_tenant_role_name UNIQUE
(
    tenant_id,
    name
)
...
-- Comments
COMMENT
ON TABLE permissions IS 'System permissions for access control';
COMMENT
ON TABLE roles IS 'User roles for RBAC';
COMMENT
ON TABLE role_permissions IS 'Mapping of permissions to roles';
COMMENT
ON TABLE user_roles IS 'Assignment of roles to users';
...
VALUES
    ...
    ('audit.view', 'View audit logs', 'audit', 'view') ON CONFLICT DO NOTHING;

-- Insert default system roles
INSERT INTO roles (id, tenant_id, name, description, is_system_role, is_active)
VALUES
    -- Global system roles
    ('10000000-0000-0000-0000-000000000001', NULL, 'SUPER_ADMIN', 'Super administrator with full system access', TRUE,
     TRUE),
    ...
    ('20000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000000', 'TENANT_ADMIN',
     'Tenant administrator', TRUE, TRUE),
    ('20000000-0000-0000-0000-000000000002', '00000000-0000-0000-0000-000000000000', 'TENANT_MANAGER', 'Tenant manager',
     TRUE, TRUE),
    ('20000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000000', 'USER', 'Regular user', TRUE,
     TRUE),
    ('20000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000000', 'VIEWER', 'Read-only viewer', TRUE,
...
FROM permissions
WHERE resource IN ('user', 'role', 'biometric', 'analytics', 'audit') ON CONFLICT DO NOTHING;

-- Assign regular user permissions
INSERT INTO role_permissions (role_id, permission_id)
SELECT '20000000-0000-0000-0000-000000000003', id
FROM permissions
WHERE name IN ('user.read', 'user.update', 'biometric.enroll', 'biometric.verify') ON CONFLICT DO NOTHING;

-- Assign SUPER_ADMIN role to system admin user
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, '10000000-0000-0000-0000-000000000001'
FROM users u
WHERE u.email = 'admin@fivucsas.local' ON CONFLICT DO NOTHING;
