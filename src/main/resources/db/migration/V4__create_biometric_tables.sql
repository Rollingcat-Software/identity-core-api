...
CREATE TABLE IF NOT EXISTS biometric_data (
    ...
    embedding vector(2622),  -- pgvector for face embeddings (VGG-Face: 2622 dimensions)
    embedding_model VARCHAR(50) DEFAULT 'VGG-Face',
    ...
    enrolled_from_device VARCHAR(100),
    ...
    CONSTRAINT fk_user_tenant CHECK (
        EXISTS (SELECT 1 FROM users u WHERE u.id = user_id AND u.tenant_id = tenant_id)
    )
);
...
CREATE TABLE IF NOT EXISTS biometric_verification_logs (
    ...
    -- Context
    verification_type VARCHAR(50),  -- 'LOGIN', 'DOOR_ACCESS', 'TRANSACTION', etc.
...
-- Comments
COMMENT ON TABLE biometric_data IS 'Encrypted biometric embeddings for users';
COMMENT ON COLUMN biometric_data.embedding IS 'pgvector embedding (2622 dimensions for VGG-Face)';
COMMENT ON COLUMN biometric_data.liveness_verified IS 'Whether liveness was checked during enrollment';
COMMENT ON TABLE liveness_attempts IS 'Log of all liveness detection attempts';
COMMENT ON TABLE biometric_verification_logs IS 'Audit log of biometric verification attempts';
...
CREATE OR REPLACE FUNCTION ensure_single_primary_biometric()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON biometric_data
FOR EACH ROW
