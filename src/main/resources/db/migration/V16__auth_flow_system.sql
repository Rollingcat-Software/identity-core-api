...
CREATE INDEX idx_tenant_auth_methods_enabled ON tenant_auth_methods(tenant_id, is_enabled) WHERE is_enabled = true;
...
CREATE INDEX idx_auth_flows_default ON auth_flows(tenant_id, operation_type, is_default) WHERE is_default = true;
CREATE UNIQUE INDEX uq_auth_flow_default ON auth_flows(tenant_id, operation_type) WHERE is_default = true AND is_active = true;
...
CREATE INDEX idx_auth_sessions_expires ON auth_sessions(expires_at) WHERE status IN ('CREATED','IN_PROGRESS');
...
CREATE INDEX idx_auth_session_steps_delegation ON auth_session_steps(delegation_token) WHERE delegation_token IS NOT NULL;
