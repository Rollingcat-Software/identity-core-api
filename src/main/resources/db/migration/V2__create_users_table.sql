...
(
    ...
    -- Constraints
    CONSTRAINT unique_tenant_email UNIQUE
(
    tenant_id,
    email
),
...
-- Comments
COMMENT
ON TABLE users IS 'Platform users with authentication credentials';
COMMENT
ON COLUMN users.password_hash IS 'BCrypt hashed password';
COMMENT
ON COLUMN users.failed_login_attempts IS 'Counter for brute force protection';
COMMENT
ON COLUMN users.is_locked IS 'Account locked due to security reasons';
