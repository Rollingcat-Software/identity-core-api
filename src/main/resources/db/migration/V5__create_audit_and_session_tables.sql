...
(
    100
) NOT NULL, -- 'LOGIN_FAILED', 'ACCOUNT_LOCKED', 'SUSPICIOUS_ACTIVITY', etc.
    severity VARCHAR
(
    20
) NOT NULL, -- 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL'
...
-- Comments
COMMENT
ON TABLE audit_logs IS 'Comprehensive audit trail of all system actions';
COMMENT
ON TABLE refresh_tokens IS 'JWT refresh tokens for maintaining user sessions';
COMMENT
ON TABLE active_sessions IS 'Currently active user sessions';
COMMENT
ON TABLE password_history IS 'Historical passwords to prevent reuse';
COMMENT
ON TABLE security_events IS 'Security-related events and incidents';

-- Function to clean up expired sessions
CREATE
OR REPLACE FUNCTION cleanup_expired_sessions()
RETURNS void AS $$
BEGIN
...
CREATE
OR REPLACE FUNCTION cleanup_old_audit_logs()
RETURNS void AS $$
BEGIN
DELETE
FROM audit_logs
